...
const CmsDrama = () => {
    ...
    const { data, isLoading, isError } = useQuery(
        ...
        async () => {
            if (searchTerm) {
                ...
                const response = await movieDataService.searchByTitle(searchedTerm, currentPage, numberOfShows);
                console.log("Search response:", response.data);
                return response.data;
            } else if (filterStatus !== 'None') {
                console.log("Filtering by status:", filterStatus);
                const response = await movieDataService.filterByStatus(filterStatus, currentPage, numberOfShows);
                return response.data;
            } else {
                const response = await movieDataService.getAllMoviesCMS(currentPage, numberOfShows);
                return response.data;
            }
        },
        {
    ...
    const handleEdit = (movieId) => {
        navigate(`/cms-drama-edit/${movieId}`);
    };
                                                    ...
                                                    <td className="w-1/6 px-4 py-2 border-b border-gray-600">
                                                        {movie.approval_status === 'APPROVED' ? (
                                                            <span className="flex items-center text-green-500">
                                                                <i className="mr-1 fas fa-check-circle"></i> Approved
                                                            </span>
                                                        ) : (
                                                            <button
                                                                className="flex items-center text-red-500 hover:text-red-600"
                                                                onClick={() => showModal(movie.movie_id)}
                                                            >
                                                                <i className="mr-1 fas fa-times-circle"></i> Unapproved
                                                            </button>
                                                        )}
                                                    </td>
